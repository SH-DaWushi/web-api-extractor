...
> 21 个工具的完整清单、平台矩阵、已知边界），见 **[docs/reference.md](docs/reference.md)**。
...
| 我的账号密码安全吗？ | **登录态文件是明文的**（本机 `auth_states/` 目录下的 json；用账号密码登录时，账号密码也会一并写入），所以它只能留在你自己的电脑上，**不要提交、同步、截图或分享**。抓包时 JSON / 表单体里的凭据值会被抹成 `***`，但 **URL 与响应体不脱敏**；生成的项目里凭据是加密保存的（仅 Windows）。详见 reference「安全设计」。 |
...
| 非 Windows 能用吗？ | 能跑，但凭据加密不可用（只在内存里，重启要重新登录），并且必须有桌面环境。 |
...
```bash
# 取源码（本项目未发布到 PyPI，只能从源码取用）
git clone https://github.com/SH-DaWushi/web-api-extractor.git
cd web-api-extractor

# 建独立环境：创建 .venv + 装依赖 + Chromium + 自检
python bootstrap.py

# 启动服务（务必用它；不要用后台任务直接跑 run_http.py）
python start_server.py              # 监听 127.0.0.1:8422/mcp
python start_server.py --status     # 查看状态
python start_server.py --stop       # 停止

# 调用工具
python mcp_call.py <tool_name> '<json-arguments>'
...
工具清单（21 个）、环境变量、数据目录、能力边界、平台矩阵、安全设计
