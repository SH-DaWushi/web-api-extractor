...
`.gitignore` 已覆盖，列在这里便于排查「为什么多了个文件」：

| 产物 | 来源 |
|---|---|
| `.mcp_session` / `args.json` | `mcp_call.py` 的会话缓存与 `@file` 传参 |
| `.server_pid` / `*.log` | `start_server.py` 的 PID 与日志 |
| `capture.jsonl` / `analysis.json` / `scripts/` | 抓包会话（在数据目录，不在仓库） |
| `auth_states/<site>.json` | 登录态，**明文**，绝不可入库 |
| `token_cache.bin` / `cred_cache.bin` | 生成的子项目的 DPAPI 加密凭据缓存 |
| `web-api-extractor-agent.zip` | `package-agent.ps1` 的产物 |
| `*.bak` | `generator.py` 重出文件时的备份 |
| `dist/` / `build/` / `*.egg-info/` / `__pycache__/` | 构建与字节码产物 |
